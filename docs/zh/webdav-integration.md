...
lalmax-nvr 提供 WebDAV 服务器功能，允许通过标准 WebDAV 协议访问摄像头录像和快照。支持只读和读写两种模式，便于与第三方系统集成。
...
> **认证说明**：WebDAV 服务器使用全局 `auth` 配置中的用户名和密码进行 HTTP Basic 认证，无需单独配置 WebDAV 凭据。
>
> ```yaml
> auth:
>   username: "admin"
...
| 字段 | 必需 | 类型 | 默认值 | 描述 |
|------|------|------|--------|------|
| `enabled` | 否 | boolean | true | 是否启用 WebDAV 服务器 |
| `path_prefix` | 否 | string | "/dav" | WebDAV URL 前缀 |
| `read_write` | 否 | boolean | false | 是否允许写入操作 |
