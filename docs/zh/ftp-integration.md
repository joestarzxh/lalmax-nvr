...
lalmax-nvr 包含内置 FTP 服务器，允许通过标准 FTP 协议访问摄像头录像和快照。支持被动模式，便于与各种 FTP 客户端和系统集成。

## 概述

- **协议**: FTP (File Transfer Protocol)
- **模式**: 被动模式 (PASV)
- **认证**: 使用全局 `auth.username` / `auth.password`
- **端口**: 2121（默认）
- **路径**: `/recordings/` 和 `/snapshots/`
- **兼容性**: 支持标准 FTP 客户端
...
> **认证说明**：FTP 服务器使用全局 `auth` 配置中的用户名和密码进行认证，无需单独配置 FTP 凭据。
>
> ```yaml
> auth:
>   username: "admin"
...
| 字段 | 必需 | 类型 | 默认值 | 描述 |
|------|------|------|--------|------|
| `enabled` | 否 | boolean | true | 是否启用 FTP 服务器 |
| `port` | 否 | integer | 2121 | FTP 服务器端口 |
| `passive_port_range` | 否 | string | "2122-2140" | 被动模式端口范围 |
...
4. **使用 SFTP 替代**：对于安全要求高的场景，考虑通过 WebDAV 或其他加密协议访问
