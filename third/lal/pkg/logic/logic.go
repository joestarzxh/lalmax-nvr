...
type ILalServer interface {
	...
	// SetHlsEnabled toggles TS-HLS generation at runtime without restarting the server.
	// When disabled, active HLS muxers are stopped but pull/pub sessions keep running.
	SetHlsEnabled(enable bool)

	// SetHlsOnDemand toggles on-demand TS-HLS slicing and its idle timeout.
	SetHlsOnDemand(onDemand bool, idleTimeoutMs int)
}
