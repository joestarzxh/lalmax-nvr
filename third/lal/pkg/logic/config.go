...
type HlsConfig struct {
	...
	OnDemandIdleTimeoutMs   int    `json:"on_demand_idle_timeout_ms"`
}
...
func LoadConfAndInitLog(rawContent []byte) *Config {
	...
	if !j.Exist("log.level") {
		config.LogConfig.Level = nazalog.LevelInfo
...
`)
	...
	if (config.HlsConfig.Enable || config.HlsConfig.EnableHttps) && !j.Exist("hls.on_demand") {
		config.HlsConfig.OnDemand = true
	}
	if (config.HlsConfig.Enable || config.HlsConfig.EnableHttps) && !j.Exist("hls.on_demand_idle_timeout_ms") {
		config.HlsConfig.OnDemandIdleTimeoutMs = 60000
	}
