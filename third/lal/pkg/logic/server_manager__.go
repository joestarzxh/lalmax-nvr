...
func (sm *ServerManager) AddCustomizePubSession(streamName string) (ICustomizePubSessionContext, error) {
	...
	// Build minimal PubStartInfo for authentication and hook notification.
	// Customize sessions (WHIP, SRT, GB28181) don't have full session info
	// at this point, but we have the stream name for ban checking.
	info := base.PubStartInfo{}
	info.StreamName = streamName
	info.AppName = "live"
	info.Protocol = "customize"

	// Authenticate before creating the session (unified auth path for all protocols)
	if err := sm.option.Authentication.OnPubStart(info); err != nil {
		return nil, err
	}

	group := sm.getOrCreateGroup("", streamName)
	sessionCtx, err := group.AddCustomizePubSession(streamName)
	if err != nil {
		return nil, err
	}

	info.SessionId = sessionCtx.UniqueKey()
	info.HasInSession = group.HasInSession()
	info.HasOutSession = group.HasOutSession()

	// Fire on_pub_start hook (unified with RTMP/RTSP)
...
func (sm *ServerManager) OnHlsAccess(streamName string) {
	sm.mutex.Lock()
	defer sm.mutex.Unlock()
