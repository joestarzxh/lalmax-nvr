...
func (sm *ServerManager) SetHlsEnabled(enable bool) {
	sm.mutex.Lock()
	sm.config.HlsConfig.Enable = enable
	sm.mutex.Unlock()
...
func (sm *ServerManager) SetHlsOnDemand(onDemand bool, idleTimeoutMs int) {
	sm.mutex.Lock()
	sm.config.HlsConfig.OnDemand = onDemand
	if idleTimeoutMs > 0 {
		sm.config.HlsConfig.OnDemandIdleTimeoutMs = idleTimeoutMs
	}
	sm.mutex.Unlock()
	...
	sm.groupManager.Iterate(func(group *Group) bool {
		group.mutex.Lock()
		if onDemand {
			group.tickHlsOnDemandIdleLocked()
		} else if group.hasInSession() {
